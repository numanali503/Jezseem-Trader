    ...
    if (storedData) {
      try {
        ...
        const filteredServices = data.filter(
          (service) => service.categoryName === "after-sales-services"
        );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full">
              {services.map((service, index) => (
                <Link
                  to={`/${service.linkURL}`}
                  key={service.id}
                  className="px-4 sm:px-6 py-4 flex flex-col h-full bg-white/50 backdrop-blur-sm rounded-lg hover:shadow-lg transition-shadow duration-300"
                >
